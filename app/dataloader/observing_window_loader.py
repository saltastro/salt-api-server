...
class ObservingWindowLoader(DataLoader):
    ...
    def batch_load_fn(self, block_ids):
        return Promise.resolve(self.get_observing_windows(block_ids))

    def get_observing_windows(self, block_ids):
        ...
        df_block_observing_windows = pd.read_sql(sql_observing_windows, con=db.engine, params=dict(block_ids=block_ids))
        ...
        def get_observing_window_content(block_id):
            observing_window = values.get(block_id)[0]
            if not observing_window:
                raise GraphQLError(
                    "There is no observing window for the block with id {block_id}".format(
                      block_id=block_id
                    )
                )
        ...
        return [
            get_observing_window_content(block_id)
